...
contract SwapTokens is Context {
    ISwapRouter public immutable swapV3Router;
    IWETH public WETH;

    constructor(address _WETH, address _swapV3Router) {
        WETH = IWETH(_WETH);
        swapV3Router = ISwapRouter(_swapV3Router);
    }
    ...
    function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _feeTier) external payable returns (uint256) {
        ...
        uint256 amountOut = swapInternal(who, sendTo, _tokenIn, tokenOut, valueCoin, _feeTier);
        if (_tokenOut == addressNull) {
    ...
    function swapInternal(address who, address _sendTo, address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _feeTier) internal returns (uint256) {
        ...
        IERC404 token = IERC404(tokenIn);
        if (_tokenIn != addressNull) {
            token.transferFrom(who, address(this), _amountIn);
        }
        token.approve(address(swapV3Router), _amountIn);
        ...
        uint256 amountOut = swapV3Router.exactInputSingle(params);
        return amountOut;
    }
}